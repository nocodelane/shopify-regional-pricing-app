(function() {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        async function applyModalCustomization() {
            ...
            } catch (e) { console.error("Modal config error:", e); }
        }

        applyModalCustomization();

        const storedRegion = getStored("regionId");
        function initVisibility() {
            if (!storedRegion) return;
            const cached = localStorage.getItem(`denied_visibility_${storedRegion}`);
            if (cached) {
                try {
                    const { products, collections } = JSON.parse(cached);
                    updateHidingStyles(products, collections);
                } catch (e) {}
            }
        }
        function updateHidingStyles(dp, dc) {
            let styleEl = document.getElementById(`pincode-dynamic-blocker`);
            if (!styleEl) { styleEl = document.createElement('style'); styleEl.id = `pincode-dynamic-blocker`; document.head.appendChild(styleEl); }
            let css = `[data-pincode-hidden="true"] { display: none !important; } `;
            if (dp) dp.forEach(h => { const s = `a[href$="/products/${h}"], a[href*="/products/${h}?"]`; css += `${s} { display: none !important; } .grid__item:has(${s}), .product-card:has(${s}) { display: none !important; }`; });
            if (dc) dc.forEach(h => { const s = `a[href$="/collections/${h}"], a[href*="/collections/${h}?"]`; css += `${s} { display: none !important; } .grid__item:has(${s}), .collection-card:has(${s}) { display: none !important; }`; });
        ...
        document.addEventListener("click", (e) => {
            if (e.target.closest('[data-open-pincode-modal]')) { e.preventDefault(); if (modal) modal.style.display = "flex"; }
            if (e.target.id === "pincode-modal-close" || e.target.id === "pincode-modal") { if (modal) modal.style.display = "none"; }
        });
    });
})();
